...
/**
 * File Name = PreRetrieveGUI.java	Version 2.0.0 Class Name = PreRetrieveGUI
 *
...
public class PreRetrieveGUI extends JFrame implements ActionListener {
    ...
    private CryptoInformation info;

    public PreRetrieveGUI(CryptoInformation info, boolean operation) {
        ...
        lblVersion = new MyJLabel("H.264", arialFont, Color.black, Color.gray);
        ...
        gbc.gridx = 3;
        gbl.setConstraints(lblMasterFile, gbc);
        panelUpper.add(lblMasterFile);
        gbc.gridx = 1;
        gbc.gridy = 2;
        gbl.setConstraints(lblVersion, gbc);
        panelUpper.add(lblVersion);
        gbc.gridx = 3;
        gbl.setConstraints(lblVersionUsed, gbc);
        panelUpper.add(lblVersionUsed);
        gbc.gridx = 1;
        gbc.gridy = 3;
        gbl.setConstraints(lblContains, gbc);
        panelUpper.add(lblContains);
        gbc.gridx = 3;
        gbl.setConstraints(lblContainsWhat, gbc);
        panelUpper.add(lblContainsWhat);
        gbc.gridx = 1;
        gbc.gridy = 4;
        gbl.setConstraints(lblCompressed, gbc);
        panelUpper.add(lblCompressed);
        gbc.gridx = 3;
        gbl.setConstraints(lblCompressedStatus, gbc);
        panelUpper.add(lblCompressedStatus);
        gbc.gridx = 1;
        gbc.gridy = 5;
        gbl.setConstraints(lblCompression, gbc);
        panelUpper.add(lblCompression);
        gbc.gridx = 3;
        gbl.setConstraints(lblCompressionRatio, gbc);
        panelUpper.add(lblCompressionRatio);
        gbc.gridx = 1;
        gbc.gridy = 6;
        gbl.setConstraints(lblEncrypted, gbc);
        panelUpper.add(lblEncrypted);
        gbc.gridx = 3;
        gbl.setConstraints(lblEncryptedStatus, gbc);
        ...
        gbc.gridx = 3;
        gbl.setConstraints(lblRequestedOperation, gbc);
        ...
        gbc.gridy = 3;
        gbl.setConstraints(panelLower, gbc);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnCancel) {
            ...
            if (lblEncryptedStatus.getText().equals("YES")) {
                ...
                boolean jn = false;
                while (jn == false) {
                    result = JOptionPane.showOptionDialog(null, message, "Encrypted zone", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[0]);

                    if (result == 1) {
                        return;
                    }
                    System.out.println("-----------------------------------------------------" + lblMasterFile.getText());
                    String PaS = new Sample().Retirve(lblMasterFile.getText());
                    ...
                    if (password.length() < 8) {
                        ...
                        JOptionPane.showMessageDialog(null, "Invalid Password Please Try Again", "Invalid password", JOptionPane.OK_OPTION);
                        jn = false;
                    } else {
                        jn = true;
                    }
    ...
    private void showFile(CryptoInformation info) {
...
class MessageDisplay extends JFrame {
    ...
    public MessageDisplay(String message, String fileName) {
        super("Retrieved message from file '" + fileName + "' - H.264 " + CPT.VERSION + "    -- By: ");
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
