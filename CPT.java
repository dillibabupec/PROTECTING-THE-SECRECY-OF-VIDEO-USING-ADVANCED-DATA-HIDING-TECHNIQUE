...
/**
 * File Name = CPT.java	Version 2.0.0 Class Name = CPT
 *
...
/**
 * * Header description: ***	In bytes: (4)+ 3+ 1+ 1+ 4= 2+ 9 Bytes. (*4 - for
 * every 1 byte in () as every 4 bytes of of master file contain 2 bits) 2 bytes
 * representing the size of the actual file 3 bytes indicate the version of the
 * software used to embed message eg. 2 0 0 1 Byte1 (4 actual bits) incicates
 * features used which include file Compression, Encryption, Message/File 1 byte
 * contains the compression ratio 4 bytes Contain the size of the data (A 32 bit
 * integer value)
**
 */
public class CPT {
    ...
    private CPT() {
        System.out.println("crypto " + VERSION + " ready...");
    }
    ...
    public static String retrieveMessage(CryptoInformation info, String password) {
        String messg = null;
        ...
        try {
            ...
            messg = new String(CryptoInformation.byteToCharArray(messageArray));
        } catch (Exception e) {
            message = "Oops!!\n Error: " + e;
            e.printStackTrace();
            return ("#FAILED#");
    ...
    public static boolean retrieveFile(CryptoInformation info, String password, boolean overwrite) {
        File dataFile = null;
        features = info.getFeatures();
    ...
    private static class AboutFrame extends JFrame implements ActionListener {
        ...
        public AboutFrame() {
            setTitle("About Cryptograph " + CPT.VERSION);
            filler1 = new JLabel(" ");
            filler2 = new JLabel(" ");
            filler3 = new JLabel(" ");
            lblTitle = new MyJLabel("Cryptography Implementation " + CPT.VERSION, new Font("Arial", Font.PLAIN, 15), YELLOW, BLACK);
            ...
            if (new File("Images/About").exists()) {
                ...
                lblImage = new MyJLabel("Cryptograph " + CPT.VERSION, new Font("Times new roman", Font.PLAIN, 50), Color.red, Color.black);
            ...
            panelAbout = UtilityOperations.createBorderedPanel(panelAbout, "About Cryptograph", 3, 3);
...
class CryptoInformation {
    ...
    public CryptoInformation(File file) {
        this.file = file;
        isEster = false;
...
class HTMLFrame extends JFrame implements HyperlinkListener {
    ...
    public HTMLFrame(String startURL, boolean isOnline) {
        super("Help - Cryptograph " + CPT.VERSION + " -- By: vinoth");

        editorPane = new JEditorPane();
        editorPane.setEditable(false);
        if (isOnline) {
            editorPane.setBackground(Color.white);
            setTitle("Cryptograph " + CPT.VERSION + " -- By: vinoth");
        }
        //else
