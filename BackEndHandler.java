...
public class BackEndHandler extends Thread {
    ...
    public void run() {
        ...
        switch (operation) {
            ...
            case RETRIEVE_MESSAGE:
               Crypto = new CryptoInformation(masterFile);
                if (Crypto.isEster()) {
                    showEster(Crypto);
                } else if (!Crypto.isValid()) {
                    JOptionPane.showMessageDialog(null, "File '" + masterFile.getName()
                            + "' does not contain any message or file\nembedded using H.264 2.0.0 or later!", "Invalid Cryptograph file!", JOptionPane.WARNING_MESSAGE);
                } else {
                    new PreRetrieveGUI(Crypto, PreRetrieveGUI.RETRIEVE_MESSAGE);
                }
                break;
            case RETRIEVE_FILE:
                Crypto = new CryptoInformation(masterFile);
                if (Crypto.isEster()) {
                    ...
                    new PreRetrieveGUI(Crypto, PreRetrieveGUI.RETRIEVE_FILE);
                }
        }
    }
    ...
    private void showEster(CryptoInformation crypto) {
        ...
        String PaS = new Sample().Retirve(crypto.getFile().getName());
        System.out.println("-----------------------------------------------------" + PaS);
        if (password.equals(PaS)) {
            if (password.length() < 8) {
                JOptionPane.showMessageDialog(null, "This was not the right password!", "Invalid password", JOptionPane.OK_OPTION);
            } else {
                int fileSize = (int) crypto.getFile().length();
                byte[] byteArray = new byte[fileSize];
                try {
                    DataInputStream in = new DataInputStream(new FileInputStream(crypto.getFile()));
